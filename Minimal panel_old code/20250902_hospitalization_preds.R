...
# ---------------- threshold selection ----------------
# pick threshold that maximizes MCC on inner out of fold predictions
best_threshold_mcc <- function(obs, p_pos, pos = "Yes", neg = "No",
                               grid = NULL, na.rm = TRUE) {
  if (na.rm) {
    keep <- !is.na(obs) & is.finite(p_pos)
    obs  <- obs[keep];      p_pos <- p_pos[keep]
  }
  y <- factor(obs, levels = c(neg, pos))
  P <- sum(y == pos); N <- sum(y == neg)
  if (length(p_pos) == 0 || P == 0 || N == 0) {
    return(list(t = 0.5, mcc = NA_real_))
  }
  p <- as.numeric(p_pos)
  p[p < 0] <- 0; p[p > 1] <- 1
  if (is.null(grid)) grid <- sort(unique(c(0, 1, p)), decreasing = FALSE)
  
  Ypos  <- as.integer(y == pos)
  Yneg  <- 1L - Ypos
  preds <- outer(p, grid, ">=")   # n x m logical
  
  TP <- colSums(preds * Ypos)
  FP <- colSums(preds * Yneg)
  FN <- sum(Ypos) - TP
  TN <- sum(Yneg) - FP
  
  denom <- sqrt((TP + FP) * (TP + FN) * (TN + FP) * (TN + FN))
  mcc   <- (TP * TN - FP * FN) / denom
  mcc[!is.finite(mcc)] <- NA_real_
  
  j <- suppressWarnings(which.max(mcc))
  if (length(j) == 0 || !is.finite(mcc[j])) {
    return(list(t = 0.5, mcc = NA_real_))
  }
  list(t = grid[j], mcc = unname(mcc[j]))
}
...
eval_fold_binary <- function(dat_tr, dat_va, spec, yvar, pos_label, algo_name, fold_tag, inner_k, verbose) {
  ...
  thr <- if (is.null(t_res$t) || !is.finite(t_res$t)) 0.5 else t_res$t
  ...
  pred <- factor(ifelse(pr[[pos_label]] >= thr, pos_label, neg_label), levels = lev)
  ...
  if (isTRUE(verbose)) {
    cat(sprintf("[Fold %s] %s | %s | tuned.t=%.3f | pos=%s, neg=%s\n",
                fold_tag, yvar, algo_name, thr, pos_label, neg_label))
  }
  m
}
